...
void del_duplicates(List *lst) {
  List e;
  List t;
  List hold = NULL;

  for(e = *lst; e != 0; e = e->next) {
    for(t = e->next; t != 0; t = t->next) {
      if (t->value == e->value) {
        hold = e->next;
        e->value = e->next->value;
        e->next = e->next->next;
...
int main(int argc, char **argv)
{
  ...
  List ret = circular(&lst);
  printf("value of first node of circular %i\n", ret->value);
